...
class _SpringBState extends State<SpringB> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Rope animation using Spring package'),
        ),
...
class SpringPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(tail.position.x, tail.position.y), 22, paint);
   canvas.drawCircle(Offset(head.position.x, head.position.y), 22, paint);
    //drawRRect(rrect., paint)Rect(Rect.fromLTRB(30, 30, 30, 30), paint);
